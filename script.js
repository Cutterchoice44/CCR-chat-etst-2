// ─── CometChat Credentials (fill these in) ─────────────────────────────
const COMETCHAT_APP_ID   = "27471907b883a58a";
const COMETCHAT_REGION   = "EU";
const COMETCHAT_AUTH_KEY = "f877c5cae80d3ec238080cad932702b12077c7f";

// ─── CometChat Initialization & UI Render ───────────────────────────
document.addEventListener('DOMContentLoaded', () => {
  ...
  .then(() => {
    ...
    let uid = localStorage.getItem('ccrChatNick') || prompt("Enter chat nickname:");
    localStorage.setItem('ccrChatNick', uid);
    const user = new CometChat.User(uid);
    return CometChat.createUser(user, COMETCHAT_AUTH_KEY)
      .then(() => CometChat.login(uid, COMETCHAT_AUTH_KEY))
      .catch(() => CometChat.login(uid, COMETCHAT_AUTH_KEY));
  })
  .then(() => {
    const GUID = "cutters_choice_chat";
    const GROUP_TYPE = CometChat.GROUP_TYPE.PUBLIC;
    const GROUP_NAME = "Cutters Choice Radio Chat";
    return CometChat.joinGroup(GUID, GROUP_TYPE, COMETCHAT_AUTH_KEY)
      .catch(err => {
        if (err.code === "ERR_GROUP_NOT_FOUND") {
          ...
          .then(() => CometChat.joinGroup(GUID, GROUP_TYPE, COMETCHAT_AUTH_KEY));
        }
        throw err;
      });
  })
  .then(() => {
    CometChatUI.renderGroup(
      {
        GUID: "cutters_choice_chat",
        groupType: CometChat.GROUP_TYPE.PUBLIC,
        name: "Cutters Choice Radio Chat"
      },
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ─── Chat nickname persistence on mobile ───────────────────────────────
  const chatFrame = document.querySelector('.chat iframe');
  if (chatFrame) {
    let nick = localStorage.getItem('ccrChatNick');
    if (!nick) {
      nick = prompt('Enter a chat nickname:');
      localStorage.setItem('ccrChatNick', nick);
    }
